...
6.1 Alle Zugriffe laufen durch **ein Zugriffs-Schloss**: Mutationen nehmen es **exklusiv** (der eine serialisierte Writer), Leser, die **mehrere** Dateien bzw. Manifeste überspannen, nehmen es **geteilt**. Dadurch ist — **innerhalb eines Prozesses** — jeder Lese- wie Schreibzugriff *als Ganzes* **atomar**: ein Leser beobachtet **nie** den Zwischenzustand einer mehrschrittigen Mutation (etwa ein Verschieben über Verzeichnisgrenzen: `rename` + zwei Manifeste). **Zusätzlich** ist jeder einzelne Datei- und Manifest-Schreibvorgang für sich **atomar** (Temp-Datei + `rename`), so dass auch ein prozess-fremder Leser nie eine halb geschriebene Datei sieht.
...
- **Nebenläufigkeit über Prozessgrenzen.** *Innerhalb* eines Prozesses sind alle Zugriffe über das Zugriffs-Schloss koordiniert (§6.1): ein serialisierter Writer und dagegen geteilt sperrende Leser, so dass jeder Zugriff als Ganzes atomar ist. **Mehrere Prozesse** auf derselben Wurzel sind hingegen **nicht** koordiniert (kein prozess-übergreifendes Schloss); zwischen ihnen trägt allein die Einzel-Datei-Atomarität (Temp-Datei + `rename`).
- **Absturz-Konsistenz mehrschrittiger Mutationen.** Dies betrifft den **Absturz** (Strom-/Prozessverlust), nicht die Nebenläufigkeit (die §6.1 in-Prozess abdeckt). Einzelne Datei- und Manifest-Schreibvorgänge sind atomar (§6.1); ein Vorgang, der *mehrere* schreibt (Verschieben über Verzeichnisgrenzen: `rename` + zwei Manifeste), ist in v1 **nicht** transaktional. Ein Absturz im Fenster hinterlässt eine reparierbare Inkonsistenz (verwaiste Datei oder Eintrag), nie Datenverlust; die Reihenfolge ist bewusst so gewählt, dass der Baum die Wahrheit bleibt und ein Index-Neubau (§8) wieder aufräumt. Ein Journal/WAL ist eine offene Betriebsentscheidung.
